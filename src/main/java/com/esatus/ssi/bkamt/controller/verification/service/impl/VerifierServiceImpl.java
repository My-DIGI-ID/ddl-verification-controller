...
public class VerifierServiceImpl implements VerifierService {
  ...
  public boolean checkMetaDataCompliance(VerificationRequestMetadata verificationRequestMetadata) {
    if (Objects.nonNull(verificationRequestMetadata)) {
      return true;
    } else {
      return false;
    }
  ...
  public Optional<Verifier> getOneByApiKey(String apiKey) {
    ...
    Optional<Verifier> verifierOptional = all.stream()
        .filter((Verifier verifier) -> bCryptPasswordEncoder.matches(apiKey, verifier.getApiKey())).findFirst();
