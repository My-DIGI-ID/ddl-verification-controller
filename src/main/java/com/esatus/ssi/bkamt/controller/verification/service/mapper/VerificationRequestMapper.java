...
public class VerificationRequestMapper {
  public VerificationRequest presentationRequestDTOToPresentationRequest(
  ...
  public VerificationRequestDTO verificationRequestToVerificationRequestDTO(VerificationRequest verificationRequest) {
    if (verificationRequest == null) {
      ...
      verificationRequestDTO.setNonce(verificationRequest.getNonce());

      return verificationRequestDTO;
    }
  }

  public List<VerificationRequestDTO> presentationRequestsToPresentationRequestsDTOs(
      List<VerificationRequest> verificationRequests) {
    return verificationRequests.stream().filter(Objects::nonNull).map(this::verificationRequestToVerificationRequestDTO)
        .collect(Collectors.toList());
  }

  public List<VerificationRequest> presentationRequestDTOsToPresentationRequests(
      List<VerificationRequestDTO> verificationRequestDTOS) {
    return verificationRequestDTOS.stream().filter(Objects::nonNull)
        .map(this::presentationRequestDTOToPresentationRequest).collect(Collectors.toList());
  }
}
